module WithRootTag
  ...
  included do
    ...
    option :root_tag_content, optional: true
  ...
  def root_tag(*args, **attributes_at_render, &block)
    ...
    if block_given?
      ...
      tag.send(root_tag_name || tag_attributes[:tag_name], root_tag_content, *args, **tag_attributes.without(:tag_name), &block)
    end
  end
  ...
  def root_tag_content
    content.presence || super
  end
end
